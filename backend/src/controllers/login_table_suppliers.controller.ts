...
export const createSupplier = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const existingSupplierById =
        await supplierService.getSupplierById(id_supplier);
      ...
      const existingSupplierByEmail =
        await supplierService.getSupplierByEmail(email);
      ...
      if (existingSupplierByNip) {
        res
          .status(409)
          .json({ error: "Dostawca o podanym numerze NIP już istnieje" });
        return;
      }
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (nip && nip !== existingSupplier.nip) {
        const existingSupplierByNip =
          await supplierService.getSupplierByNip(nip);
