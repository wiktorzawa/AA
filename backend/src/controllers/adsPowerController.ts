...
const CreateProfilePayloadSchema = z
  .object({
    ...
    country: z
      .string()
      .refine(isValidCountry, { message: "Invalid country code." })
      .optional(),
...
const UpdateProfileBodySchema = z
  ...
  .refine((data) => Object.keys(data).length > 0, {
    message:
      "Request body cannot be empty. At least one field to update must be provided.",
...
const UpdateGroupBodySchema = z
  ...
  .strict()
  .refine(
    (data) => data.group_name !== undefined || data.remark !== undefined,
    {
...
export class AdsPowerController {
   ...
   */
  public checkAdsPowerApi = asyncHandler(
   ...
   */
  public handleCreateProfile = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (
        ...
        console.error(
          "AdsPower API returned an error during profile creation:",
          apiResponse,
        );
   ...
   */
  public listAdsPowerProfiles = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (apiResponse && apiResponse.code === 0) {
        ...
        console.error(
          "AdsPower API returned an error when listing profiles:",
          apiResponse,
        );
        res.status(apiResponse?.code === -1 || !apiResponse ? 503 : 500).json({
          success: false,
  ...
  public regroupAdsPowerProfiles = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const bodyValidationResult = RegroupProfilesBodySchema.safeParse(
        req.body,
      );
      if (!bodyValidationResult.success) {
        res.status(400).json({
   ...
   */
  public startAdsPowerBrowser = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const paramsValidationResult = UserIdParamSchema.safeParse(req.params);
      if (!paramsValidationResult.success) {
        res.status(400).json({
      ...
      const queryValidationResult = StartBrowserQuerySchema.safeParse(
        req.query,
      );
   ...
   */
  public stopAdsPowerBrowser = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const paramsValidationResult = UserIdParamSchema.safeParse(req.params);
      if (!paramsValidationResult.success) {
        res.status(400).json({
  ...
  public createAdsPowerGroup = asyncHandler(
    async (
      req: Request<object, object, CreateGroupPayload>,
      res: Response,
    ): Promise<void> => {
   ...
   */
  public listAdsPowerGroups = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const queryValidationResult = GroupPaginationQuerySchema.safeParse(
        req.query,
      );
      ...
      if (apiResponse && apiResponse.code === 0 && apiResponse.data) {
        ...
        console.error(
          "AdsPower API returned an error when listing groups:",
          apiResponse,
        );
        res.status(apiResponse?.code === -1 || !apiResponse ? 500 : 400).json({
          success: false,
   ...
   */
  public updateAdsPowerGroup = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
   ...
   */
  public deleteAdsPowerGroups = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
